...
// API service
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_BASE_URL || 'https://fiyatiq.wxcodesign.com/api';

const api = {
  hizliTahmin: async (data: any) => {
    const response = await fetch(`${API_BASE_URL}/hizli-tahmin`, {
      method: 'POST',
  ...
  detayliTahmin: async (data: any) => {
    const response = await fetch(`${API_BASE_URL}/detayli-tahmin`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    ...
    <CardContent className="space-y-4">
      <div className="flex justify-between items-center p-4 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <span className="text-3xl font-bold text-blue-600">
          {result.ortalama_fiyat.toLocaleString('tr-TR', {
            style: 'currency',
            currency: 'TRY',
      ...
      <div className="grid grid-cols-2 gap-4 text-center">
        <div className="p-3 bg-green-50 rounded-lg border border-green-200">
          ...
          <div className="text-xl font-semibold text-green-800">
            {result.tahmini_fiyat_min.toLocaleString('tr-TR', {
              style: 'currency',
              currency: 'TRY',
              minimumFractionDigits: 0
            })}
        ...
        <div className="p-3 bg-red-50 rounded-lg border border-red-200">
          ...
          <div className="text-xl font-semibold text-red-800">
            {result.tahmini_fiyat_max.toLocaleString('tr-TR', {
              style: 'currency',
              currency: 'TRY',
              minimumFractionDigits: 0
            })}
      ...
      <div>
        <h4 className="font-semibold text-gray-700 mb-2 flex items-center">
          <Sparkles className="w-5 h-5 mr-2 text-yellow-500" /> Rapor
        </h4>
        <div
          className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md border prose"
          dangerouslySetInnerHTML={{ __html: result.rapor }}
        />
      </div>
      <div>
        <h4 className="font-semibold text-gray-700 mb-2 flex items-center">
          <FileText className="w-5 h-5 mr-2 text-indigo-500" /> Pazar Analizi
        </h4>
        <div
          className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md border prose"
          dangerouslySetInnerHTML={{ __html: result.pazar_analizi }}
        />
      </div>
      <p className="text-xs text-center text-gray-500 pt-2">
        Analiz Tarihi: {new Date(result.analiz_tarihi).toLocaleString('tr-TR')}
      </p>
    </CardContent>
  </Card>
);
...
const WelcomeCard = () => (
  <Card className="bg-white/60 backdrop-blur-sm border-blue-200/80 shadow-lg">
    <CardHeader>
      <CardTitle className="text-2xl font-bold text-gray-800">
        FiyatIQ'ya Hoş Geldiniz!
      </CardTitle>
    </CardHeader>
    <CardContent className="space-y-4">
      <p className="text-gray-600">
        Aracınızın gerçek piyasa değerini en doğru şekilde öğrenmek için yapay
        zeka destekli analiz motorumuzu kullanın.
      </p>
      <ul className="space-y-3 text-sm text-gray-700">
        <li className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-gray-600">
              Analizlerimiz, internetteki güncel ilanları tarayarak aracınızın
              hasarsız piyasa değerini referans alır.
            </p>
          </div>
        </li>
        <li className="flex items-start gap-3">
          ...
          <div>
            <strong className="font-semibold">
              Detaylı Değer Kaybı Analizi
            </strong>
            <p className="text-gray-600">
              Aracınızdaki her bir boyalı veya değişen parçanın fiyata etkisini
              TL cinsinden hesaplayarak şeffaf bir rapor sunarız.
            </p>
          </div>
        </li>
        <li className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-gray-600">
              Yapay zeka, tüm verileri birleştirerek size özel, kapsamlı bir
              fiyat analizi ve pazar durumu raporu oluşturur.
            </p>
          </div>
        </li>
      </ul>
      <p className="text-center pt-2 text-sm text-gray-500">
        Başlamak için soldaki formu doldurun!
      </p>
    </CardContent>
  </Card>
);
...
export default function HomePage() {
  ...
  const addHasarKaydi = () => {
    setFormData(prev => ({
      ...prev,
      hasar_detaylari: [...prev.hasar_detaylari, { parca: '', durum: '' }]
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const apiFunc =
        activeTab === 'hizli' ? api.hizliTahmin : api.detayliTahmin;
      ...
      <header className="py-6">
        <div className="container mx-auto flex items-center justify-center gap-3">
          ...
          <h1 className="text-4xl font-bold tracking-tight text-gray-900">
            Fiyat<span className="text-blue-600">IQ</span>
          </h1>
        </div>
        <p className="text-center text-lg text-gray-600 mt-2">
          Yapay Zeka Destekli Araç Fiyatlandırma Asistanınız
        </p>
      </header>
                  ...
                  <TabsContent value="hizli">
                    <div className="space-y-4 animate-fade-in">
                      <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <Select
                          name="yil"
                          onValueChange={v => handleSelectChange('yil', v)}
                          value={formData.yil}
                          required
                        >
                        ...
                        <Select
                          name="yakit_tipi"
                          onValueChange={v =>
                            handleSelectChange('yakit_tipi', v)
                          }
                        ...
                        <Select
                          name="vites_tipi"
                          onValueChange={v =>
                            handleSelectChange('vites_tipi', v)
                          }
                        ...
                        >
                          ...
                          <SelectContent>
                            {['Otomatik', 'Manuel', 'Yarı Otomatik'].map(y => (
                              <SelectItem key={y} value={y}>
                                {y}
                              </SelectItem>
                            ))}
                      ...
                      <Button
                        type="submit"
                        className="w-full !mt-6 bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg"
                        disabled={loading}
                      >
                        {loading ? (
                  ...
                  <TabsContent value="detayli">
                    <div className="space-y-4 animate-fade-in">
                      ...
                      <textarea
                        ...
                        placeholder="Ekstra Bilgiler (Örn: Bakımları yeni yapıldı, sigara içilmemiş...)"
                        rows={2}
                      ...
                      <div className="space-y-3 pt-2">
                        ...
                        {formData.hasar_detaylari.map((hasar, index) => (
                          <div
                            key={index}
                            className="flex items-center gap-2 p-2 bg-gray-100/80 rounded-md"
                          >
                            <Input
                              name="parca"
                              placeholder="Örn: Kaput"
                              value={hasar.parca}
                              onChange={e => handleHasarChange(index, e)}
                              className="flex-grow"
                            />
                            <Select
                              onValueChange={v =>
                                handleHasarDurumChange(index, v)
                              }
                              value={hasar.durum}
                            >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              size="icon"
                              onClick={() => removeHasarKaydi(index)}
                        ...
                        <Button
                          type="button"
                          variant="outline"
                          onClick={addHasarKaydi}
                          className="w-full border-dashed"
                        >
                          <PlusCircle className="h-4 w-4 mr-2" /> Hasar Kaydı
                          Ekle
                        </Button>
                      </div>
                      <Button
                        type="submit"
                        className="w-full !mt-6 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-lg"
                        disabled={loading}
                      >
                        {loading ? (
                          <Loader2 className="mr-2 h-5 w-5 animate-spin" />
